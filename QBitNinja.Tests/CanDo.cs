    ...
    public class CanDo
    {
        ...
        public void CanGetTransaction()
        {
            using(var tester = ServerTester.Create())
            {
                ...
                Assert.Contains(response.ReceivedCoins.OfType<ColoredCoin>(), c => c.Amount == new AssetMoney(gold, 1000) && c.TxOut.ScriptPubKey == bob.ScriptPubKey);

                //Goldguy receives 5BTC
                Assert.Contains(response.ReceivedCoins.OfType<Coin>(), c => c.Amount == Money.Coins(5.0m) && c.TxOut.ScriptPubKey == goldGuy.ScriptPubKey);
                ...
                Assert.Contains(response.SpentCoins.OfType<ColoredCoin>(), c => c.Amount == new AssetMoney(gold, 1000) && c.TxOut.ScriptPubKey == bob.ScriptPubKey);

                //Bob received 1500 silver
                Assert.Contains(response.ReceivedCoins.OfType<ColoredCoin>(), c => c.Amount == new AssetMoney(silver, 1500) && c.TxOut.ScriptPubKey == bob.ScriptPubKey);

                //Silverguy received 600 gold
                Assert.Contains(response.ReceivedCoins.OfType<ColoredCoin>(), c => c.Amount == new AssetMoney(gold, 600) && c.TxOut.ScriptPubKey == silverGuy.ScriptPubKey);
                //Bob received 400 gold
                Assert.Contains(response.ReceivedCoins.OfType<ColoredCoin>(), c => c.Amount == new AssetMoney(gold, 400) && c.TxOut.ScriptPubKey == bob.ScriptPubKey);

                //Silverguy received 37BTC
                Assert.Contains(response.ReceivedCoins.OfType<Coin>(), c => c.Amount == Money.Coins(37.0m) && c.TxOut.ScriptPubKey == silverGuy.ScriptPubKey);
            }
        }
        ...
        public void CanBroadcastTransaction()
        {
            using(var tester = ServerTester.Create())
            {
                var tx = CreateRandomTx(tester.Network);
                ...
                tx = CreateRandomTx(tester.Network);
        ...
        public void CanGetBlock()
        {
            using(var tester = ServerTester.Create())
            {
                ...
                Assert.True(response2.Block.ToBytes().SequenceEqual(block.ToBytes()));
                var prevBlock = response2.Block.GetHash();

                response2 = tester.SendGet<GetBlockResponse>("blocks/tip");
                Assert.Null(response2.ExtendedInformation);
                Assert.True(response2.Block.ToBytes().SequenceEqual(block2.ToBytes()));
                response2 = tester.SendGet<GetBlockResponse>("blocks/" + prevBlock + "+1");
                Assert.True(response2.Block.ToBytes().SequenceEqual(block2.ToBytes()));
                ...
                var txx = tester.Network.Consensus.ConsensusFactory.CreateTransaction();
                txx.Outputs.Add(new TxOut(Money.Coins(55m), Script.Empty));
                tester.ChainBuilder.Broadcast(txx);
                var block3 = tester.ChainBuilder.EmitBlock();
                tester.UpdateServerChain();

                //Can get extended info
                ...
                Assert.Equal(1, response2.ExtendedInformation.TransactionCount);
                ///////
            }
        }
        ...
        public void HDKeysetsAreCorrectlyTracked()
        {
            using(var tester = ServerTester.Create())
            {
                ...
                Assert.Single(balance.Operations);
                ...
                Assert.Contains(balance.Operations, op => op.Amount == Money.Coins(1.0m));
                Assert.Contains(balance.Operations, op => op.Amount == Money.Coins(1.1m));
